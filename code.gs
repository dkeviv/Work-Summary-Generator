...
function createConfigurationCard(e = {}) {
  ...
  whoWhenSection.addWidget(CardService.newTextInput()
  ...
  whoWhenSection.addWidget(CardService.newTextParagraph()
    .setText("<i><b>Note:</b> For security, this script can only analyze the data of the person running it. The email must match the current user.</i>"));

  whoWhenSection.addWidget(CardService.newDatePicker()
...
function createReportAction(e) {
  ...
  // Dates from DatePicker come in ms since epoch
  ...
  if (!fullName || !startDateMs || !endDateMs || !weightsStr) {
    return CardService.newActionResponseBuilder().setNotification(CardService.newNotification().setText('Error: All fields are required.')).build();
  }
  
  const userInputs = {
      userEmail: userEmail, fullName: fullName, startDate: new Date(startDateMs), endDate: new Date(endDateMs),
      userDomain: userEmail.split('@')[1], weights: {}
  };
  ...
  if (weightsArr.length !== 4 || weightsArr.some(isNaN) || weightsArr.reduce((a, b) => a + b, 0) !== 100) {
     return CardService.newActionResponseBuilder().setNotification(CardService.newNotification().setText('Error: Invalid weights. Please enter 4 numbers that sum to 100.')).build();
  }
  ...
  try {
    ...
    const analysis = analyzeData(rawData, userInputs.weights, userInputs.userDomain);
...
function fetchData(startDate, endDate, userEmail) {
  ...
  // Fetch Gmail Threads (sent items)
  ...
  for (const thread of threads) {
    ...
    activities.push({
      ...
      description: message.getPlainBody().substring(0, 500),
      attendees: [message.getTo(), message.getCc()].join(',').split(',').filter(e => e.trim() !== '')
    });
  }
  return activities;
}

function analyzeData(activities, weights, userDomain) {
  const analysis = {
    activities: [],
    metrics: {},
    distribution: {
    ...
    workPatterns: {byHour: Array(24).fill(0), commSplit: {internal: 0, external: 0}},
    recommendations: []
  };
...
function insertDistributionTable(body, distribution, total) {
  ...
  for (const cat in map) {
    ...
    if (item.count > 0) {
      data.push([map[cat], `${((item.count/total)*100).toFixed(1)}%`, `${(item.totalScore/item.count).toFixed(1)}%`]);
    }
  }
  createStyledTable(body, data);
}
