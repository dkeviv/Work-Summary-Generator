...
function createConfigurationCard() {
  ...
  // Add LLM option
  var llmSection = CardService.newCardSection().setHeader('Advanced AI Options');
  var llmSelect = CardService.newSelectionInput()
    .setFieldName('use_llm')
    .setTitle('Use Local LLM (Ollama) for Summary Generation?')
    .setType(CardService.SelectionInputType.DROPDOWN)
    .addItem('No', 'false', true)
    .addItem('Yes', 'true', false);
  llmSection.addWidget(llmSelect);
  builder.addSection(llmSection);
...
function createReportAction(e) {
  try {
    ...
    var userInputs = {
      ...
      useLLM: formInputs.use_llm ? (formInputs.use_llm[0] === 'true') : false
    };
...
function processJob() {
  ...
  try {
    ...
    var reportUrl;
    if (userInputs.useLLM) {
      // Call local LLM (Ollama) via web API
      var llmSummary = callOllamaForSummary(analysis, userInputs);
      reportUrl = generateReportWithLLM(llmSummary, userInputs);
    } else {
      reportUrl = generateReport(analysis, userInputs);
    }
...
// Call Ollama LLM for summary generation
function callOllamaForSummary(analysis, userInputs) {
  try {
    var payload = {
      model: 'llama2',
      prompt: 'Generate an executive-ready work summary based on this data: ' + JSON.stringify(analysis)
    };
    var options = {
      method: 'post',
    ...
    var response = UrlFetchApp.fetch('http://localhost:11434/api/generate', options);
    var result = JSON.parse(response.getContentText());
    return result.response || 'LLM summary not available.';
  } catch (err) {
    return 'Error calling LLM: ' + err;
  }
}

// Generate report using LLM summary
function generateReportWithLLM(llmSummary, userInputs) {
  var doc;
  try {
    var docName = 'Performance Summary (LLM) - ' + userInputs.fullName + ' (' + userInputs.startDate.toLocaleDateString() + ')';
    ...
    body.appendParagraph('Executive Summary (Generated by LLM)').setAttributes(CONFIG.STYLE.H1);
    body.appendParagraph(llmSummary);
    doc.saveAndClose();
    return doc.getUrl();
  } catch (error) {
    if (doc) {
    ...
    throw new Error('Failed to generate LLM report: ' + error.message);
  }
}
